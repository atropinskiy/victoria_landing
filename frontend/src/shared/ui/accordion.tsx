...
function AccordionContent({ className, children, ...props }: AccordionPrimitive.Panel.Props) {
  return (
    <AccordionPrimitive.Panel
      ...
      className="h-(--accordion-panel-height) overflow-hidden text-sm transition-[height] duration-400 ease-in-out data-ending-style:h-0 data-starting-style:h-0"
      {...props}
    >
      <div
        className={cn(
          "[&_a]:hover:text-foreground pt-0 pb-2.5 [&_a]:underline [&_a]:underline-offset-3 [&>p:not(:last-child)]:mb-4",
          className
        )}
      >
        {children}
